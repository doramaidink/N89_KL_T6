...
const TU_KHOA_VI_PHAM = [
  //chửi tục
  "địt", "đụ", "lồn", "cặc", "đéo", "địt mẹ", "đụ mẹ","con cặc",
...
function chuanHoaText(text = "") {
  ...
  const noAccent = boDauTiengViet(cleaned);

  return {
    raw,
    noAccent,
    full: `${cleaned} ${noAccent}`
  };
}
...
function kiemTraTuKhoa(text = "") {
  const { raw, noAccent } = chuanHoaText(text);

  const tuTrung = TU_KHOA_VI_PHAM.filter((tu) => {
    const tuRaw = tu.toLowerCase();
    const tuNoAccent = boDauTiengViet(tuRaw);

    const regexRaw = new RegExp(
      `\\b${escapeRegex(tuRaw)}\\b`,
      "iu"
    );

    // luôn kiểm tra bản gốc
    const matchRaw = regexRaw.test(raw);

    let matchNoAccent = false;

    // chỉ dùng noAccent cho từ dài
    if (tuNoAccent.length >= 4) {
      const regexNoAccent = new RegExp(
        `\\b${escapeRegex(tuNoAccent)}\\b`,
        "iu"
      );

      matchNoAccent = regexNoAccent.test(noAccent);
    }

    return matchRaw || matchNoAccent;
  });

  if (!tuTrung.length) return null;
  ...
  return {
    ...
    lyDo: "Nội dung vi phạm tiêu chuẩn cộng đồng",
